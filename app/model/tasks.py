...
from app.models import Sms, Gps, Ownership
...
tdelta = datetime.timedelta(seconds=300)
...
def model_run(begin):
    ...
    with app.app_context():
        phonenumber = db.session.query(Ownership).order_by(
            Ownership.phone.desc()).first()
        texting = str(phonenumber.phone)
                                ...
                                if (jsarr["label_id"] == "/m/01j4z9" and jsarr["probability"] == maxPrediction):
                                    ...
                                    try:
                                        ...
                                        if not q:
                                            ...
                                            time_obj = Sms(last_sent_chainsaw, last_sent_vehicle)
                                            ...
                                            chainsaw_send_count = (db.session.query(Sms).filter(
                                                Sms.last_sent_chainsaw > count_sms_since).all())
                                            print(chainsaw_send_count)
                                    ...
                                    try:
                                        ...
                                        if not q:
                                            ...
                                            time_obj_2 = Sms(last_sent_chainsaw, last_sent_vehicle)
